...
class datacardClass:
	...
	##full simulation toy study
	weight = ROOT.RooRealVar('weight','event weight',0.01)
	...
	name = ''
        ...
        rfv_n_CB = ROOT.RooFormulaVar(name,"("+params[name]+")",ROOT.RooArgList(self.MH))
	...
	##---------------------------------------------------------------bbH
	name = "signalCB_bbH_{0}_{1}_{2}_{3}".format(self.year,self.fs_name,self.cate_name,self.tag_name)
	if (self.MassOnly) : signalCB_bbH = ROOT.RooDoubleCB(name, name, CMS_zz4l_mass, CMS_zz4l_mean_sig_NoConv, self.getVariable(rfv_MassErr,rfv_sigma_CB, self.bIncludingError),rfv_alpha_CB,rfv_n_CB, rfv_alpha2_CB, rfv_n2_CB)
	if (not self.MassOnly and self.FFT) :
		DetectorReso_bbH = ROOT.RooDoubleCB(name+'_reso',name,CMS_zz4l_mass, CMS_zz4l_mean_sig_Conv, self.getVariable(rfv_MassErr,rfv_sigma_CB, self.bIncludingError), rfv_alpha_CB,rfv_n_CB, rfv_alpha2_CB, rfv_n2_CB)
	...
	##---------------------------------------------------------------------------bkgs
        ...
        ## Reducible backgrounds ##							#updated
        bkg_zjets = ROOT.RooLandau();
        print "zjets mass shape"
	name = 'zx_mean_'+self.year+'_'+self.fs_name+'_'+self.cate_name+'_'+self.tag_name
	p0_zjets = ROOT.RooRealVar(name,name,float(params[name]))
	name = 'zx_sigma_'+self.year+'_'+self.fs_name+'_'+self.cate_name+'_'+self.tag_name
	p1_zjets = ROOT.RooRealVar(name,name,float(params[name]))
	name = "bkg_zjetsTmp_"+self.year+"_4mu_"+self.cate_name+"_"+self.tag_name
	bkg_zjets = ROOT.RooLandau(name,name,CMS_zz4l_mass,p0_zjets,p1_zjets)
	##------------------------------------------------------------------------------------->1D shapes completed
	...
	## -------------------------------------- MELA 2D PDFS ----------------------------------------- ##
        ...
        TemplateName = "sigTemplatePdf_bbH_{0}_{1}_{2}_{3}".format(self.year,self.fs_name,self.cate_name,self.tag_name)
        sigTemplatePdf_bbH = ROOT.RooHistPdf(TemplateName,TemplateName,ROOT.RooArgSet(CMS_zz4l_mass,D),sigTempDataHist)
        TemplateName = "sigTemplatePdf_bbH_Up_{0}_{1}_{2}_{3}".format(self.year,self.fs_name,self.cate_name,self.tag_name)
        sigTemplatePdf_bbH_Up = ROOT.RooHistPdf(TemplateName,TemplateName,ROOT.RooArgSet(CMS_zz4l_mass,D),sigTempDataHist_Up)
	...
	#qqzzbkgTemplatePdf = sigTemplatePdf_ggH.Clone(TemplateName)
	...
	TemplateName = "bkgTemplatePdf_ggzz_{0}_{1}_{2}_{3}".format(self.year,self.fs_name,self.cate_name,self.tag_name)
        ...
        ##### ZX KD
        zxbkgTemplate_beforeFix = fi.Get('zx_'+self.year+'_'+self.fs_name+'_'+self.cate_name+'_'+self.tag_name+'_flat')
	zxbkgTemplate_beforeFix.SetDirectory(0)
        zxbkgTemplate_Up_beforeFix = fi.Get('zx_'+self.year+'_'+self.fs_name+'_'+self.cate_name+'_'+self.tag_name+'_flat')
	zxbkgTemplate_Up_beforeFix.SetDirectory(0)
        zxbkgTemplate_Down_beforeFix = fi.Get('zx_'+self.year+'_'+self.fs_name+'_'+self.cate_name+'_'+self.tag_name+'_flat')
	zxbkgTemplate_Down_beforeFix.SetDirectory(0)
        zxbkgTemplate = self.histogramBinFix(zxbkgTemplate_beforeFix,zxbkgTemplate_beforeFix.GetName())
        zxbkgTemplate_Up = self.histogramBinFix(zxbkgTemplate_Up_beforeFix,zxbkgTemplate_Up_beforeFix.GetName())
        zxbkgTemplate_Down = self.histogramBinFix(zxbkgTemplate_Down_beforeFix,zxbkgTemplate_Down_beforeFix.GetName())
...
#           rfvSigRate_ggzz = ROOT.RooFormulaVar("ggzz_norm","1+@0*@1",ROOT.RooArgList(JES,ggzzJES))
        ...
        #dataFileName = "CMSdata/RunII_OldEBE/"+self.year+"/hzz_"+self.fs_name+"_"+self.year+"_"+self.type_name+"_"+self.cate_name+"_"+self.totcate_name+".root"
        dataFileName = "CMSData_Agosto2023/"+self.year+"/hzz_"+self.fs_name+"_"+self.year+"_"+self.type_name+"_"+self.cate_name+"_"+self.totcate_name+".root"

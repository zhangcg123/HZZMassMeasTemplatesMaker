...
def runcombineformass(file_, computer, withsyst, toyn, floatothers, ranges):
	...
	taskname = '--task-name '+outputname+'_fullsimtoys_'+toyn+'_'+withsyst+'_'+computer
	fixed_options1 = '-M MultiDimFit -d '+outputname+'.root -m 125.0 -P MH --setParameters MH=125.0 --floatOtherPOIs='+floatothers+' --algo=singles -D '+toyn
	fixed_options2 = '-M MultiDimFit -d '+outputname+'.root -m 125.0 -P MH --setParameters MH=125.0 --floatOtherPOIs='+floatothers+' --robustFit=0 --algo=grid --points=20 -D '+toyn
...
def runcombineforwidth(file_, computer, withsyst, toyn, poi, floatothers, ranges):
	...
	outputlogname = outputname+'_fullsimtoys_'+toyn+'_'+withsyst+'_'+computer+'.log'
	taskname = '--task-name '+outputname+'_fullsimtoys_'+toyn+'_'+withsyst+'_'+computer
	fixed_options1 = '-M MultiDimFit -d '+outputname+'.root -m 125.0 -P '+poi+' --setParameters HiggsDecayWidth=0.004,MH=125.0 --floatOtherPOIs='+floatothers+' --robustFit=0 --algo=grid --points=20 -D '+toyn
	fixed_options2 = '-M MultiDimFit -d '+outputname+'.root -m 125.0 -P '+poi+' --setParameters HiggsDecayWidth=0.004,MH=125.0 --floatOtherPOIs='+floatothers+' --robustFit=0 --algo=singles -D '+toyn
	if withsyst == 'withsyst' and computer == 'local':
	...
	if withsyst == 'withsyst' and computer == 'condor':
		combline = 'combineTool.py '+outputrootname+' '+fixed_options1+' '+opt3+' '+opt4+' '+opt5+' '+taskname+' | tee '+outputlogname
	if withsyst == 'withoutsyst' and computer == 'condor':
		combline = 'combineTool.py '+outputrootname+' '+fixed_options1+' '+opt2+' '+opt3+' '+opt4+' '+opt5+' '+taskname+' | tee '+outputlogname

...
def runcombineforwidth(file_,computer, withsyst, totcat, seed, poi, floatothers, ranges):
	...
	outputname = file_+ '_GENERATEONLY_widthmodelfor'+poi+'_floatothers'+floatothers+'_width_'+str(ranges['HiggsDecayWidth'][0])[0:6].replace('.','p')+'_'+str(ranges['HiggsDecayWidth'][1])[0:6].replace('.','p')+'_MH_'+str(ranges['MH'][0])[0:6].replace('.','p')+'_'+str(ranges['MH'][1])[0:6].replace('.','p')
	...
	outputlogname = outputname+'_'+events+'x'+totcat+'_seed'+str(seed)+'_'+withsyst+'_'+computer+'.log'
	taskname = '--task-name '+outputname+'_'+events+'x'+totcat+'_seed'+str(seed)+'_'+withsyst+'_'+computer
	fixed_options = '-M GenerateOnly --saveToys -d '+outputname+'.root -m 125.38 --setParameters HiggsDecayWidth=0.004,MH=125.38,r=1.0 --X-rtd TMCSO_PseudoAsimov='+events+' --X-rtd TMCSO_AdaptivePseudoAsimov=0 -s '+str(rm)
	# opt1 = '-v 3' opt2 = '--freezeParameters allConstrainedNuisances' opt3 = '--job-mode condor' opt4 = '--split-points 1'	
	if withsyst == 'withsyst' and computer == 'local':
	...
	if withsyst == 'withsyst' and computer == 'condor':
		combline = 'combineTool.py '+outputrootname+' '+fixed_options+' '+taskname+' | tee '+outputlogname
	if withsyst == 'withoutsyst' and computer == 'condor':
		combline = 'combineTool.py '+outputrootname+' '+fixed_options+' '+opt2+' '+opt3+' '+taskname+' | tee '+outputlogname

...
def runcombineformass(file_, computer, withsyst, totcat, seed, floatothers, ranges):
	...
	t2wline = 'text2workspace.py ' + file_ + '.txt --X-allow-no-signal --X-allow-no-background -P HiggsAnalysis.CombinedLimit.PhysicsModel:floatingHiggsMass --PO higgsMassRange='+str(ranges['MH'][0])+','+str(ranges['MH'][1])+' --PO=muAsPOI -o ' + outputname + '.root'
	...
	fixed_options1 = '-M GenerateOnly --saveToys -d '+outputname+'.root -m 125.0 --setParameters MH=125.0,r=1.0 -t -1 --X-rtd TMCSO_PseudoAsimov='+events+' --X-rtd TMCSO_AdaptivePseudoAsimov=0 -s '+str(rm)
...
def runcombineforwidth(file_,computer, withsyst, totcat, seed, poi, floatothers, ranges):
	...
	outputlogname = outputname+'_'+events+'x'+totcat+'_seed'+str(seed)+'_'+withsyst+'_'+computer+'.log'
	taskname = '--task-name '+outputname+'_'+events+'x'+totcat+'_seed'+str(seed)+'_'+withsyst+'_'+computer
	fixed_options = '-M GenerateOnly --saveToys -d '+outputname+'.root -m 125.0 --setParameters HiggsDecayWidth=0.004,MH=125.0 --expectSignal=1 -t -1 --X-rtd TMCSO_PseudoAsimov='+events+' --X-rtd TMCSO_AdaptivePseudoAsimov=0 -s '+str(rm)
...
def runcombineformasswidth(file_,computer, withsyst, totcat, seed):
	...
	t2wline = 'text2workspace.py ' + file_ + '.txt --X-allow-no-signal --X-allow-no-background -P HiggsAnalysis.CombinedLimit.FloatingHiggsWidth:floatingHiggsWidth --PO higgsWidthRange='+str(ranges['HiggsDecayWidth'][0])+','+str(ranges['HiggsDecayWidth'][1])+' --PO higgsMassRange='+str(ranges['MH'][0])+','+str(ranges['MH'][1])+' -o ' + outputname + '.root'

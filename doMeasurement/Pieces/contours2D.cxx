...
void contour2D(TString xvar, int xbins, float xmin, float xmax, TString yvar, int ybins, float ymin, float ymax, float smx=1.0, float smy=1.0, TFile *fOut=0, TString path="./", TString name="contour2D") {
    ...
    //}
    TLegend* leg = new TLegend(.7,.72,.9,.85);
    ...
    L3->SetLineStyle(1);
    L3->SetLineWidth(3);
    leg->AddEntry(L3,"-2 #Delta NLL=2.3");
    TLine* L4 = new TLine();
    L4->SetLineColor(1);
    L4->SetLineStyle(2);
    L4->SetLineWidth(3);
    leg->AddEntry(L4,"-2 #Delta NLL=5.9");
    leg->Draw("SAME");
    c.SaveAs(path + name + "doublesmooth_2D_contours.png");
}

void contours2D(TString path, TString name) {
	
	contour2D("MH",20,124.85,125.9,"HiggsDecayWidth",20,0,1.0,1.0,1.0,0,path,name);	

...
void multirun(int top, int bottom,TH2F* h ){			//this function should be self-contained
	...
	for(int i=top; i<bottom; i++){
		...
		if(vars_i[0]==1 && vars_i[1]==1 && vars_f[0]<0.35 && vars_f[1]<0.35 && vars_f[2]<4 && vars_f[3]<4 && vars_f[4]<5000 && vars_f[5]<5000 && vars_f[6]<100 && vars_f[7]<100 && vars_f[8]>32 && vars_f[9]>20 && -2.5<vars_f[10] && vars_f[10]<2.5 && -2.5<vars_f[11] && vars_f[11]<2.5 && 60<vars_f[12] && vars_f[12]<120){
			h->Fill(vars_f[4],vars_f[5]);
		}
	}
}
void multibranchgetentry(){
	...
	for(int i=0; i<ncores; i++){
		...
		TH2F* h = new TH2F("h_"+i_s,"",xbins,xlow,xhigh,ybins,ylow,yhigh);
		histos.push_back(h);
	...
	//if(fitsetting!="")bwxdcb(histos.at(0),varsname.at(0),outputdir,filename);
	TCanvas c("c","",1400,1000);
	c.cd();
	histos.at(0)->Draw("colz");
	c.SaveAs(TString(outputdir) + TString(filename) + ".png");

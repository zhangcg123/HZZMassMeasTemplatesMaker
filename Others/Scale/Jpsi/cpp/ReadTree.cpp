...
int main(int argc, char* argv[]){
	...
	TString subname;
	if(fs == "2e"){
		if(isdata=="mc" && year == "2017" )subname="crab_DYJetsToLL_amcatnlo_20UL17_2e_slimmed.root";
		if(isdata=="data" && year == "2017")subname="crab_SingleElectron_Run2017_UL2017_2e_slimmed.root";
		if(isdata=="mc" && year == "2018")subname="crab_DYJetsToLL_amcatnlo_20UL18_2e_slimmed.root";
		if(isdata=="data" && year == "2018")subname="crab_EGamma_Run2018_UL2018_2e_slimmed.root";
		if(isdata=="mc" && year == "2016pre")subname="crab_DYJetsToLL_amcatnlo_20UL16pre_2e_slimmed.root";
		if(isdata=="data" && year == "2016pre")subname="crab_SingleElectron_Run2016_UL2016HIPM_2e_slimmed.root";
		if(isdata=="mc" && year == "2016post")subname="crab_DYJetsToLL_amcatnlo_20UL16post_2e_slimmed.root";
		if(isdata=="data" && year == "2016post")subname="crab_SingleElectron_Run2016_UL2016_2e_slimmed.root";
	}
	if(fs == "2mu"){
		if(isdata=="mc" && year == "2016pre" ) subname = "crab_JpsiToMuMu_JpsiPt8_2016APV_2mu_slimmed.root";
		if(isdata=="data" && year == "2016pre" ) subname = "crab_SingleMuon2016HIPM_2mu_slimmed.root";
		if(isdata=="mc" && year == "2016post" ) subname = "crab_JpsiToMuMu_JpsiPt8_2016_2mu_slimmed.root";
		if(isdata=="data" && year == "2016post" ) subname = "crab_SingleMuonRun2016_2mu_slimmed.root";
		if(isdata=="mc" && year == "2018" )subname="crab_JpsiToMuMu_JpsiPt8_2017_2mu_slimmed.root";
		if(isdata=="data" && year == "2018")subname="crab_SingleMuon_Run18_2mu_slimmed.root";
		if(isdata=="mc" && year == "2017")subname="crab_JpsiToMuMu_JpsiPt8_2018_2mu_slimmed.root";
		if(isdata=="data" && year == "2017")subname="crab_SingleMuon_Run17_2mu_slimmed.root";

	}
	TString inputpath = "/eos/user/c/chenguan/Tuples/LeptonScale_20SummerUltraLegacy/newbranch/" + subname;
	...
	for(int i=0; i<etabinN; i++){
		...
		TH1D* h_eta_pos = new TH1D("eta_pos_"+i_s,"",int(100),stof(low),stof(high));
		TH1D* h_eta_neg = new TH1D("eta_neg_"+i_s,"",int(100),stof(low),stof(high));
		TH1D* h_phi_pos = new TH1D("phi_pos_"+i_s,"",int(100),stof(low),stof(high));
		TH1D* h_phi_neg = new TH1D("phi_neg_"+i_s,"",int(100),stof(low),stof(high));
		...
		if(i<ptbinN){
			TH1D* h_pt_pos = new TH1D("pt_pos_"+i_s,"",int(100),stof(low),stof(high));
			TH1D* h_pt_neg = new TH1D("pt_neg_"+i_s,"",int(100),stof(low),stof(high));
...
void readtree_(TString year, TString fs, TString inputpath, TString outputpath, TString isdata, TString mass_name, TString binned, string low, string high, float init_mean, float buffer,
	...
	float Sip1, Sip2, RelIso1, RelIso2, weight, weight1, weight2, d01, d02, zpt, zy;
	float eventweight, pileupweight, genweight, preweight, preweightecal, preweightmuon;
	...
	*/
	...
	if( fs == "2mu" && isdata == "mc" && massname == "mass2mu_ewk2" ){
		t->SetBranchStatus("mass2mu_corr",1);
	...
	*/
	...
	//TFile* f_zpty = new TFile("/afs/cern.ch/work/c/chenguan/CMSRunII-HiggsMassMears/HZZMassMeasTemplatesMaker/Others/Scale/Electron/cpp/reweight/lut/2017.root","READ");
	//TH2D* lut_zpty = (TH2D*)f_zpty->Get("LUT");
	...
	for(int i=0; i<t->GetEntries(); i++){
		...
		if( RelIso1<0.35 &&RelIso2<0.35 && Sip1<4 && Sip2<4 &&  trigger_cut && ecalDriven_cut && pt_cut && abs(eta1)<eta_high && abs(eta2)<eta_high && mass<stof(high)&&mass>stof(low) ){
			//zptyw = 1.0;
			...
			/*if(isdata=="mc"){
				if(zpt>500||abs(zy)>2.4){
					...
					int zptbin = zpt_ax->FindBin(zpt);
					int zybin = zy_ax->FindBin(zy);
					zptyw = lut_zpty->GetBinContent(zptbin,zybin);
				}
			}
			*/
			//weight = weight * zptyw;
...
void dofit(TString year,TString outputpath, vector<TH1D*> &histos, string low, string high, TString isdata, TString mass_name,float init_mean, float buffer, TString k){	
	...
	for(int i=0; i<histos.size(); i++){
		...
		frame->SetTitle("");
		TH1D* h_tmp = (TH1D*)histos.at(i)->Clone();
	...
	if(GEN==false){	
	for(int i=0; i<histos.size(); i++){
		double init_sigma=0.02, init_a1=1.0, init_a2=1.0, init_n1=10.0, init_n2=10.0, init_tau=0, init_fsig=1;
		...
		RooRealVar x("x","",stof(low),stof(high));
		//x.setBins(int(((stof(high)-stof(low))*1000.0)/10.0),"cache");
		RooRealVar PDGmZ("PDGmZ","PDGmZ",peak);
		...
		RooRealVar m("m","m",init_mean,3.0,3.15);
		RooRealVar s("s","s",init_sigma,0,0.05);
		...
		RooAddPdf Model("Model","Model",DCB,bkg,fsig);
		...
		//}
		double left = s.getVal()*(-a1.getVal())+m.getVal();
		double right = 	s.getVal()*(a2.getVal())+m.getVal();
		...
		TString peaktofile_s = to_string(m.getVal());
		...
		frame->Draw();
		latex->DrawLatex(0.7,0.99,"#chi^{2}/DOF="+ch2_s);
		latex->DrawLatex(0.7,0.9,"peak="+peaktofile_s);
		latex->DrawLatex(0.7,0.8,"mean="+peak_s+"+/-"+peakerr_s);
		latex->DrawLatex(0.7,0.7,"sigma="+sig_s);
		latex->DrawLatex(0.7,0.6,"a1="+a1_s);
		latex->DrawLatex(0.7,0.5,"n1="+n1_s);
		latex->DrawLatex(0.7,0.4,"a2="+a2_s);
		latex->DrawLatex(0.7,0.3,"n2="+n2_s);
		latex->DrawLatex(0.7,0.2,"entries="+entries_s);

...
void RochUnc(TString year_, TString plotpath_){ 
...
void makedataset(TString year, vector<RooDataSet*> &h_data){
	...
	//	if(i>1000)break;
		...
		if(passedFullSelection==1&&finalState==1&&abs(eta1)<2.4&&abs(eta2)<2.4&&abs(eta3)<2.4&&abs(eta4)<2.4){
...
void dofit(vector<RooDataSet*> &h_data, vector<TString> &names){//dataset, 0-99:stat. 100:ewk, 101:zpt, 102:dm, 103:def
	...
	for(int k =0; k<2; k++){		
		...
		latex->DrawLatex(0.9,0.95,"Stat.");
		latex->SetTextColor(0);
		latex->DrawLatex(0.9,0.9,"EWK");
		latex->SetTextColor(4);
		latex->DrawLatex(0.9,0.85,"Zpt");
		latex->SetTextColor(5);
		latex->DrawLatex(0.9,0.8,"deltaM");
		latex->SetTextColor(1);
		latex->DrawLatex(0.9,0.75,"Default(EWK2)");

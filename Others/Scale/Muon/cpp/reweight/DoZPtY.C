...
void DoZPtY(TString year, TString plotpath){
	...
	TH2D* h_2d_data = new TH2D("h_2d_data","",500,0,500,100,-2.4,2.4);
	TH2D* h_2d_mc = new TH2D("h_2d_mc","",500,0,500,100,-2.4,2.4);

	test_(1, h_data_pt, h_data_y, h_2d_data, year);
	test_(0, h_mc_pt, h_mc_y, h_2d_mc, year);

	TLegend* legend1 = new TLegend(.7,.9,.6,.8);
	...
	TLegend* legend2 = new TLegend(.7,.9,.6,.8);
	...
	TH2D* LUT = new TH2D("LUT","",500,0,500,100,-2.4,2.4);
	LUT->SetStats(0);
	for(int i=1; i<=500; i++){
		for(int j=1; j<=100; j++){
...
void test_(bool isData, TH1D* h_pt, TH1D* h_y, TH2D* h_2d, TString year){
	...
	TFile* f = new TFile();
	if(year=="2017"&&isData==1)f = new TFile("/eos/user/c/chenguan/Tuples/LeptonScale_20SummerUltraLegacy/tmp/crab_SingleElectron_Run2017_UL2017_2e_slimmed.root");
	if(year=="2017"&&isData==0)f = new TFile("/eos/user/c/chenguan/Tuples/LeptonScale_20SummerUltraLegacy/tmp/crab_DYJetsToLL_amcatnlo_20UL17_2e_slimmed.root");

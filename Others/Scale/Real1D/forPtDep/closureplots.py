...
def makeplot(mcnegpoints, mcpospoints, mcnegerrors, mcposerrors, datanegpoints, datapospoints, datanegerrors, dataposerrors, k, plotpath,what):
	...
	g_mc_neg = ROOT.TGraphErrors(len(x), numpy.array(x), numpy.array(mcnegpoints), numpy.array(xerr), numpy.array(dummy))
	g_mc_pos = ROOT.TGraphErrors(len(x), numpy.array(x), numpy.array(mcpospoints), numpy.array(xerr), numpy.array(dummy))
	g_dt_neg = ROOT.TGraphErrors(len(x), numpy.array(x), numpy.array(datanegpoints), numpy.array(xerr), numpy.array(dummy))
	g_dt_pos = ROOT.TGraphErrors(len(x), numpy.array(x), numpy.array(datapospoints), numpy.array(xerr), numpy.array(dummy))
	...
	if what == 'mean' or what == 'peak':
		max_ = max(numpy.amax(mcnegpoints),numpy.amax(mcnegpoints))*1.001
		min_ = min(numpy.amin(mcnegpoints),numpy.amin(mcnegpoints))*0.999
	elif what == 'sigma':
		max_ = 3
		min_ = 0
	...
	legend = ROOT.TLegend(.34,.84,.999,.999)
	legend.SetTextSize(0.1)
	legend.SetLineWidth(0)
	legend.SetFillColor(0)
	legend.SetBorderSize()
	legend.AddEntry(g_mc_pos,'mc:gen')
	#legend.AddEntry(g_dt_pos,'mc:gen')

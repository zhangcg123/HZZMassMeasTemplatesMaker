...
def mklut(year, ismc, evaluationplotpath, True, lookuptableplotpath):
	files = glob.glob(evaluationplotpath + '/_*txt')
	m1dict = []	
	etabounds = [0.0]
	pterrbounds = [0.0]
	for f in files:
		lambda_ = float(f.split('/')[-1].strip('.txt').split('_')[6])
		etalow = float(f.split('/')[-1].strip('.txt').split('_')[2]) 
		etabounds.append(etalow)
		etahigh = float(f.split('/')[-1].strip('.txt').split('_')[3])
	...
	etabounds = list(dict.fromkeys(etabounds))
	etabounds.sort()	
	pterrbounds.append(1)
	pterrbounds = list(dict.fromkeys(pterrbounds))
	pterrbounds.sort()	
...
#output dir
dirclass = DirTree()
dirclass.mkrootdir(year+'_'+ismc+'_allbins_muon_ebe_70_110_withexp_setbin300_closurefits')
path = dirclass.root
...
#first 3 bins
p1 = Process(target=multirun, args=(year,evaluationplotpath,'0,0.4,0,0.011', ismc))
p2 = Process(target=multirun, args=(year,evaluationplotpath,'0,0.4,0.011,1', ismc))
p3 = Process(target=multirun, args=(year,evaluationplotpath,'0.4,0.9,0,0.011', ismc))

...
def run(year,ismc,path,step,index,low,high):
	print './Closure2Electrons.exe '+year+' '+ismc+' '+path+' '+step+' '+str(index)+' '+str(low)+' '+str(high)	
	os.system('./Closure2Electrons.exe '+year+' '+ismc+' '+path+' '+step+' '+str(index)+' '+str(low)+' '+str(high))

year=sys.argv[1]
ismc=sys.argv[2]
dirs.mkrootdir('test_remake_ele_ebe'+year+'_'+ismc)
path = dirs.root
os.system('./Closure2Electrons.exe '+year+' '+ismc+' '+path + ' 1 0 0 0')# separate events evenly

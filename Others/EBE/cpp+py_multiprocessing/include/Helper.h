...
RooDataHist* MakeDataSet(TTree* t, bool isEcal, double abseta_low, double abseta_high, double deltapt_low, double deltapt_high, double lambda){
	...
	TH2D* histo = new TH2D("histo","",100,60,120,100,0,15);
	if(recursion==false){
		for(int i=0; i<totEvents; i++){
			if(i%1000000==0)cout<<i<<"/"<<totEvents<<endl;
			t->GetEntry(i);
			//if(i>100000)break;
			if(mass<120 && mass>60 && lep1_ecalDriven==ecal1 && lep2_ecalDriven==ecal2 && Tight1==1 && Tight2==1 && Iso1 < 0.35 && Iso2 < 0.35){
				if( (pterr1/pt1) < deltapt_high && (pterr1/pt1) > deltapt_low && (pterr2/pt2) < deltapt_high && (pterr2/pt2) > deltapt_low &&
					...
					histo->Fill(mass,masserr,weight_);
				}
			}
		}
	}
		...
		for(int i=0; i<totEvents; i++){
			...
			t->GetEntry(i);
			//if(i>100000)break;
			if(mass<120 && mass>60 && Tight1==1 && Tight2==1 && Iso1 < 0.35 && Iso2 < 0.35 ){

...
void GetBounds_(TString plotpath, int fs, TString type, TString tag, TString year, int cates){
	...
	if(BASE=="20UL")f = new TFile("/afs/cern.ch/work/c/chenguan/CMSRunII-HiggsMassMears/HZZMassMeasTemplatesMaker/Others/Slimmer/OutPutTrees/"+year+"/GluGluHToZZTo4L_M125_"+year+"_skimmed.root");
	...
	for(int i=0; i<cates; i++){
		...
		TH1D* h = new TH1D("h"+i_s,"",100,0,0.1);
		hists.push_back(h);
	}
